...
import { accounts, progams, spl_tokens } from "../config";
    ...
    const handleBridgeTokenToNFT = async () => {
      ...
      try {
            ...
            if(!publicKey||!wallet){
              ...
              const transaction= fairLaunch.methods.swapToFungible22().accounts(accounts).rpc();
             console.log("WOW")
              setStatus('success');
...
{balances["nft"] && <p>NFT balance: {balances["nft"]}</p>}
        ...
        <IonLabel>From</IonLabel>
            ...
            <IonSegmentButton value="token" disabled={balances["token"]==="0"} color='tertiary' onClick={()=>{setFrom('token')}}>
            <IonText color='tertiary'>
                TOKEN TO NFT
                </IonText>

                </IonSegmentButton>
            <IonSegmentButton value={'nft'} disabled={balances["nft"]==="0"} onClick={()=>{setFrom('nft')}}>
                <IonText color='tertiary'>
                NFT TO TOKEN
                </IonText>
                </IonSegmentButton>
          ...
          <IonInput type="number" value={amount} onIonChange={handleAmountChange} placeholder="Enter amount" />
          {status === 'loading' && <p>Bridging...</p>}
          {status === 'success' && result && (
            <div>
              <p>Transaction successful</p>
              <p>Transaction signature: {result.transactionSignature}</p>
              <a href={result.urls.solanaExplorerUrl} target='_blank' rel='noreferrer'>Solana Explorer</a>
            </div>
          )}
